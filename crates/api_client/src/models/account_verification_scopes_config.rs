/*
 * afrodite-backend
 *
 * Dating app backend API
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct AccountVerificationScopesConfig {
    #[serde(rename = "profile_age_range", skip_serializing_if = "Option::is_none")]
    pub profile_age_range: Option<models::AccountVerificationPlatforms>,
    #[serde(rename = "profile_name", skip_serializing_if = "Option::is_none")]
    pub profile_name: Option<models::AccountVerificationPlatforms>,
    #[serde(rename = "security_content", skip_serializing_if = "Option::is_none")]
    pub security_content: Option<models::AccountVerificationPlatforms>,
}

impl AccountVerificationScopesConfig {
    pub fn new() -> AccountVerificationScopesConfig {
        AccountVerificationScopesConfig {
            profile_age_range: None,
            profile_name: None,
            security_content: None,
        }
    }
}

