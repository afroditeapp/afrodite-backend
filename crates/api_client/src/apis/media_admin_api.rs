...
/// struct for typed errors of method [`get_security_content_admin_info`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetSecurityContentAdminInfoError {
    Status401(),
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_security_content_admin_info(configuration: &configuration::Configuration, aid: &str) -> Result<models::SecurityContentAdminInfo, Error<GetSecurityContentAdminInfoError>> {
    ...
    let uri_str = format!("{}/media_api/security_content_admin_info/{aid}", configuration.base_path, aid=crate::apis::urlencode(p_path_aid));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
