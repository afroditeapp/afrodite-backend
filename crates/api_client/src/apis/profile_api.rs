...
/// struct for typed errors of method [`get_query_available_profile_attributes`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetQueryAvailableProfileAttributesError {
    Status401(),
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_query_available_profile_attributes(configuration: &configuration::Configuration, profile_attribute_query: models::ProfileAttributeQuery) -> Result<models::ProfileAttributeQueryResult, Error<GetQueryAvailableProfileAttributesError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetQueryAvailableProfileAttributesError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
