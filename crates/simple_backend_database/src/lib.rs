...
impl DatabaseHandleCreator {
    ...
    pub async fn create_read_handle_from_config(config: &SimpleBackendConfig, index: usize, name: &'static str) -> Result<(DbReadHandle, DbReadCloseHandle), DataError> {
        let info = config.databases().get(index).ok_or(DataError::MatchingDatabaseNotFoundFromConfig.report())?;
