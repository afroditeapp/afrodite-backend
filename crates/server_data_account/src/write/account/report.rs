...
use model::{AccountIdInternal, CustomReportId, CustomReportType, ReportTypeNumber, ReportTypeNumberInternal, UpdateReportResult};
use server_data::{
...
impl WriteCommandsAccountReport<'_> {
    ...
    pub async fn report_custom_report_boolean(
        ...
        let custom_report_type_number = custom_report_id.to_report_type_number_value()
            .map_err(|e| DataError::NotAllowed.report().attach_printable(e))?;

        let custom_report_type = self.config().custom_reports().and_then(|v| v.index_with_id(custom_report_id)).map(|r| r.report_type);
        ...
        let reports = self
            .db_read(move |mut cmds| cmds.common().report().get_all_detailed_reports(creator, target, ReportTypeNumberInternal::CustomReport(custom_report_type_number), components))
            .await?;
        if reports.len() >= ReportTypeNumber::MAX_COUNT {
            return Ok(UpdateReportResult::too_many_reports());
        }

        let report_with_same_value = reports.iter().find(|v| v.report.content.custom_report.as_ref().and_then(|v| v.boolean_value) == Some(value));
        ...
        db_transaction!(self, move |mut cmds| {
            cmds.account().report().insert_custom_report_boolean(creator, target, custom_report_type_number, value)?;
            Ok(())
        })?;
