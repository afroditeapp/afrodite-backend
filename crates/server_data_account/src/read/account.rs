...
impl ReadCommandsAccount<'_> {
    ...
    pub async fn email_address(
        &self,
        id: AccountIdInternal,
    ) -> Result<Option<EmailAddress>, DataError> {
        self.db_read(move |mut cmds| cmds.account().data().email_address(id))
            .await
            .into_error()
    }
    ...
    pub async fn account_verification_data(
        &self,
        id: AccountIdInternal,
    ) -> Result<AccountVerificationDataInternal, DataError> {
        self.db_read(move |mut cmds| cmds.account().verification().account_verification_data(id))
            .await
            .into_error()
    }
