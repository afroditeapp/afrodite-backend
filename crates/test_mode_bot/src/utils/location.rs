...
impl LocationConfigUtils for LocationConfig {
    fn generate_random_location(
        ...
        let x_percent = generate_u32() as f64 / u32::MAX as f64;
        let y_percent = generate_u32() as f64 / u32::MAX as f64;

        let y_len = self.latitude_top_left - self.latitude_bottom_right;
        let x_len = self.longitude_bottom_right - self.longitude_top_left;

        let random_latitude = self.latitude_bottom_right + (y_len * y_percent);
        let random_longitude = self.longitude_bottom_right + (x_len * x_percent);
