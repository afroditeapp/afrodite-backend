...
struct State {
    ...
    seetaface_threshold: Option<f64>,
}
...
impl FaceDetector {
    pub fn new(image_process_config: &ImageProcessingConfig) -> Result<Self, ImageProcessError> {
        let Some(config) = image_process_config.file().seetaface.clone() else {
        ...
        Ok(Self {
            state: Some(State {
                model,
                config,
                seetaface_threshold: image_process_config.dynamic().seetaface_threshold,
            }),
        })
    }
    ...
    pub fn detect_face(&self, data: GrayImage) -> Result<bool, ImageProcessError> {
        ...
        let Some(threshold) = state.seetaface_threshold else {
            return Ok(false);
        };
        ...
        let result =
            std::panic::catch_unwind(|| -> Result<Vec<rustface::FaceInfo>, ImageProcessError> {
                ...
                model.set_score_thresh(threshold);
