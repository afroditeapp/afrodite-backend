...
use model::{AccessibleAccount, AccountId, DemoModeConfirmLoginResult, DemoModeId, DemoModeLoginResult, DemoModeLoginToken, DemoModePassword, DemoModeToken};
use simple_backend_utils::{ContextExt, IntoReportFromString};
use tokio::sync::RwLock;

use crate::app::{GetAccounts, GetConfig, ReadData};
...
impl <T: PartialEq + Clone> TokenState<T> {
    pub fn new(token: T) -> Self {
        Self { token, created: std::time::Instant::now() }
    }
...
impl DemoModeManager {
    pub fn new(info: Vec<DemoModeConfig>) -> Result<Self, DataError> {
        let states: Vec<DemoModeState> = info.into_iter().map(|info| DemoModeState {
            info,
            locked: false,
            stage1_token: None,
        ...
        Ok(Self { state: Arc::new(RwLock::new(State { states })) })
    }

    pub async fn stage0_login(&self, password: DemoModePassword) -> Result<DemoModeLoginResult, DataError> {
    ...
    async fn stage0_password_exists(&self, password: &DemoModePassword) -> Result<Option<IndexOrLocked>, DataError> {
        self.get_index_or_locked(|s| Ok(s.info.password_stage0 == password.password)).await
    }

    async fn stage1_password_exists(&self, password: &DemoModePassword) -> Result<Option<IndexOrLocked>, DataError> {
        self.get_index_or_locked(|s| Ok(s.info.password_stage1 == password.password)).await
    }

    async fn stage1_token_exists(&self, token: &DemoModeLoginToken) -> Result<Option<IndexOrLocked>, DataError> {
        self.get_index_or_locked(|s| Ok(s.stage1_token_equals(token)?)).await
    }

    pub async fn demo_mode_token_exists(&self, token: &DemoModeToken) -> Result<DemoModeId, DataError> {
        let result = self.get_index_or_locked(|s| Ok(s.token_equals(token)?)).await?;
    ...
    pub async fn accessible_accounts_if_token_valid(&self, token: &DemoModeToken) -> Result<AccessibleAccountsInfo, DataError> {
        let r = self.state.read().await;
        let state = r.states.iter().enumerate().find(|(i, state)| {
            match state.token_equals(token) {
                Ok(true) => true,
                Ok(false) |
                Err(_) => false,
            }
        ...
        if let Some((_, state)) = state {
            if state.info.access_all_accounts {
                ...
                let accounts: Vec<AccountId> = state.info.accessible_accounts.iter().map(|v| AccountId::new(*v)).collect();
                Ok(AccessibleAccountsInfo::Specific{ config_file_accounts: accounts, demo_mode_id: state.info.database_id })
            }
        } else {
    ...
    pub async fn stage1_login(&self, password: DemoModePassword, token: DemoModeLoginToken) -> Result<DemoModeConfirmLoginResult, DataError> {
    ...
    async fn get_index_or_locked(&self, get_action: impl Fn(&DemoModeState) -> Result<bool, DataError> ) -> Result<Option<IndexOrLocked>, DataError> {
        let r = self.state.read().await;
...
pub enum AccessibleAccountsInfo {
    All,
    Specific{ config_file_accounts: Vec<AccountId>, demo_mode_id: DemoModeId },
}

impl AccessibleAccountsInfo {
    pub async fn into_accounts<S: ReadData>(self, state: &S) -> crate::result::Result<Vec<AccountId>, DataError> {
        let (accounts, demo_mode_id) = match self {
            ...
            AccessibleAccountsInfo::Specific { config_file_accounts, demo_mode_id } =>
                (config_file_accounts, demo_mode_id),
        };

        let related_accounts = state.read().account().demo_mode_related_account_ids(demo_mode_id).await?;
    ...
    pub async fn with_extra_info<S: ReadData + GetConfig + GetAccounts>(self, state: &S) -> crate::result::Result<Vec<AccessibleAccount>, DataError> {
    ...
    pub async fn contains<S: ReadData>(&self, account: AccountId, state: &S) -> crate::result::Result<(), DataError> {
        let (accounts, demo_mode_id) = match self {
            AccessibleAccountsInfo::All => return Ok(()),
            AccessibleAccountsInfo::Specific { config_file_accounts, demo_mode_id } => (config_file_accounts, demo_mode_id),
        };
