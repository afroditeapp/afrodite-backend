...
use model::{
    AccountId, AccountIdInternal, ContentId, SecurityImage, PendingSecurityImage,
};
...
pub fn security_image_router(s: crate::app::S) -> Router {
    ...
    Router::new()
        .route(PATH_GET_SECURITY_IMAGE_INFO, get(get_security_image_info::<S>))
        .route(PATH_PUT_SECURITY_IMAGE_INFO, put(put_security_image_info::<S>))
        .route(PATH_GET_PENDING_SECURITY_IMAGE_INFO, get(get_pending_security_image_info::<S>))
        .route(PATH_PUT_PENDING_SECURITY_IMAGE_INFO, put(put_pending_security_image_info::<S>))
        .route(DELETE_PENDING_SECURITY_IMAGE_INFO, delete(delete_pending_security_image_info::<S>))
        .with_state(s)
}
