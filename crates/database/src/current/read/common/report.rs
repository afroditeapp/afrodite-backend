...
use model::{AccountId, AccountIdInternal, ReportContent, ReportDetailed, ReportDetailedInfo, ReportDetailedWithId, ReportIdDb, ReportInternal, ReportProcessingState, ReportTypeNumber};
...
impl CurrentReadCommonReport<'_> {
    fn get_all_internal_reports(
        ...
        let values: Vec<(AccountId, AccountId, ReportIdDb, ReportProcessingState)> = common_report
        ...
        let values = values.into_iter().map(|(creator, target, report_id, state)| {
            ReportInternal {
