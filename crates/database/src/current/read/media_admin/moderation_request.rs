...
use model::{AccountIdInternal, MediaModerationRequestRaw, ModerationQueueNumber, ModerationRequestId, NextQueueNumberType};
...
impl<C: ConnectionProvider> CurrentSyncReadMediaAdminModerationRequest<C> {
    ...
    pub fn get_next_active_moderation_request(
        &mut self,
        with_queue_type: ModerationQueueNumber,
        ...
        let data = queue_entry::table
            ...
            .filter(queue_entry::queue_type_number.eq(with_queue_type))
            .first(self.conn())
            .optional()
