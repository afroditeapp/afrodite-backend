...
impl UpdateManager {
    ...
    pub async fn run(mut self, mut quit_notification: ServerQuitWatcher) {
        let manager = self
            ...
            .cloned()
            .map(|config| UpdateManagerInternal {
...
impl UpdateManagerInternal {
    async fn init(&self) -> Result<(), UpdateError> {
    ...
    async fn handle_message(&self, message: &SoftwareUpdateTaskType) {
        let result = match message.clone() {
            SoftwareUpdateTaskType::Download => self.software_download().await,
            SoftwareUpdateTaskType::Install(info) => self.software_install(info).await,
        };
    ...
    async fn software_download(&self) -> Result<(), UpdateError> {
        self.set_internal_state_to(SoftwareUpdateState::Downloading)
            .await;
    ...
    async fn software_install(&self, info: SoftwareInfo) -> Result<(), UpdateError> {
        self.set_internal_state_to(SoftwareUpdateState::Installing)
            .await;
    ...
    async fn software_download_impl(&self) -> Result<(), UpdateError> {
        ...
        self.update_dir()
            .remove_downloaded_backend_and_info_json()
            .await?;

        self.internal_state.lock().await.downloaded = None;

        github_api
            .download_asset(&asset, self.update_dir().downloaded_backend_path())
            .await?;
    ...
    async fn software_install_impl(&self, info: SoftwareInfo) -> Result<(), UpdateError> {
        ...
        let backend_binary =
            if let Some(archive_file_path) = &self.config.github.archive_backend_binary_path {
        ...
        self.backend_utils()
            .replace_backend_binary(&backend_binary)
            .await
            .change_context(UpdateError::BackendUtils)?;

        UpdateDirUtils::save_info_json(&info, self.update_dir().installed_backend_info_json_path())
            .await?;
...
impl UpdateDirUtils<'_> {
    ...
    fn extracted_backend_path(&self) -> PathBuf {
        self.create_update_dir_if_needed().join("extracted_backend")
    }
    ...
    pub async fn downloaded_backend_info(&self) -> Result<Option<SoftwareInfo>, UpdateError> {
        Self::read_and_parse_info(self.downloaded_backend_info_json_path()).await
    }

    pub async fn installed_backend_info(&self) -> Result<Option<SoftwareInfo>, UpdateError> {
        Self::read_and_parse_info(self.installed_backend_info_json_path()).await
    }
    ...
    async fn read_and_parse_info(path: PathBuf) -> Result<Option<SoftwareInfo>, UpdateError> {
        ...
        let info = serde_json::from_str(&info).change_context(UpdateError::InvalidInput)?;
        Ok(Some(info))
    }

    pub async fn remove_downloaded_backend_and_info_json(&self) -> Result<(), UpdateError> {
    ...
    pub async fn save_info_json(
        ...
        let serialized =
            serde_json::to_string_pretty(info).change_context(UpdateError::Serialize)?;
