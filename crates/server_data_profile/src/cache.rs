...
use server_data::cache::profile::CachedProfile;
use server_data::cache::{CacheEntryCommon, CacheError};

use server_data::db_manager::{InternalReading, InternalWriting};
...
impl <R: InternalReading> CacheReadProfile for R {
    async fn read_cache_profile_and_common<T, Id: Into<AccountId>>(
        ...
        self.cache().read_cache(id, |e| {
            let p = e.profile.as_deref().ok_or(CacheError::FeatureNotEnabled.report())?;
    ...
    async fn read_cache_profile_and_common_for_all_accounts(
        ...
        self.cache().read_cache_for_all_accounts(|e| {
            let p = e.profile.as_deref().ok_or(CacheError::FeatureNotEnabled.report())?;
            cache_operation(p, &e.common);
            Ok(())
        }).await
...
impl <I: InternalWriting> CacheWriteProfile for I {
    ...
    async fn write_cache_profile_and_common<T, Id: Into<AccountId>>(
        ...
        self.cache().write_cache(id, |e| {
            let p = e.profile.as_deref_mut().ok_or(CacheError::FeatureNotEnabled.report())?;
