...
macro_rules! define_db_read_command {
    ($struct_name:ident) => {
        impl<C: server_data::read::ReadCommandsProvider> $struct_name<C> {
            ...
            {
                self.db_read_raw(|conn| cmd(database_profile::current::read::CurrentSyncReadCommands::new(conn))).await
            }
        }
    };
}
...
macro_rules! define_db_transaction_command {
    ($struct_name:ident) => {
        impl<C: server_data::write::WriteCommandsProvider> $struct_name<C> {
            ...
            {
                self.cmds.write_cmds().db_transaction_raw(|conn| cmd(database_profile::current::write::CurrentSyncWriteCommands::new(conn))).await
            }
        }
    };
}
