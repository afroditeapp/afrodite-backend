...
impl WriteCommandsProfileAdminVerification<'_> {
    ...
    pub async fn change_profile_name_verified_value(
        ...
        let value_already_set = if moderator_is_bot {
            profile_state.profile_name_verified == value
        } else {
            profile_state.profile_name_verified_manual == value
        };
        ...
        db_transaction!(self, move |mut cmds| {
            ...
            if moderator_is_bot {
                cmds.profile_admin()
                    .verification()
                    .change_profile_name_verified_value(account, value)?;
            } else {
                cmds.profile_admin()
                    .verification()
                    .change_profile_name_verified_manual_value(account, value)?;
            }
        ...
        self.write_cache_profile(account.as_id(), |p| {
            if moderator_is_bot {
                p.state.profile_name_verified = value;
            } else {
                p.state.profile_name_verified_manual = value;
            }
            p.update_profile_version_uuid(modification.version);
            p.state.profile_edited_time = modification.time;
            Ok(())
