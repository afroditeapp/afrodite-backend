    ...
    pub async fn profile(
        ...
        let (
            ...
            if name_update_detected {
                cmds.profile_admin()
                    .verification()
                    .change_profile_name_verified_value(id, None)?;
                cmds.profile_admin()
                    .verification()
                    .change_profile_name_verified_manual_value(id, None)?;
            }
            ...
            let profile_text_moderation_state_update = if text_update_detected {
                Some(
                    cmds.profile()
                        .moderation()
                        .reset_profile_text_moderation_state(id, profile_data.ptext.is_none())?,
                )
            } else {
                None
            };
        ...
        self.write_cache_profile(id.as_id(), |p| {
            ...
            if name_update_detected {
                p.state.profile_name_verified = None;
                p.state.profile_name_verified_manual = None;
            }
