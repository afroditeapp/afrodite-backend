...
/// struct for typed errors of method [`check_api_key`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CheckApiKeyError {
    Status404(),
...
pub async fn check_api_key(configuration: &configuration::Configuration, api_key: crate::models::ApiKey) -> Result<crate::models::AccountIdLight, Error<CheckApiKeyError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/internal/check_api_key", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CheckApiKeyError> = serde_json::from_str(&local_var_content).ok();
