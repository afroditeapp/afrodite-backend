...
/// Get profile image
pub async fn get_image_fixed(configuration: &configuration::Configuration, account_id: &str, content_id: &str) -> Result<Vec<u8>, Error<GetImageError>> {
...
pub async fn put_image_to_moderation_slot_fixed(configuration: &configuration::Configuration, slot_id: i32, body: Vec<u8>) -> Result<crate::models::ContentId, Error<PutImageToModerationSlotError>> {
    let local_var_configuration = configuration;
