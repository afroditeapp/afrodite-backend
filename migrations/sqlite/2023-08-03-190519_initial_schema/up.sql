...
CREATE TABLE IF NOT EXISTS account_permissions(
    ...
    admin_edit_profile_name_verified_value       BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    admin_view_login                             BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    admin_find_account_by_email_address          BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    admin_server_shutdown                        BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS account_email_change(
    ...
    email_change_verification_token BLOB               NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS account_email_verification_limits(
    account_id              INTEGER PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS account_email_change_limits(
    account_id              INTEGER PRIMARY KEY NOT NULL,
    monthly_email_count     SMALLINT           NOT NULL DEFAULT 0,
    monthly_limit_reset_unix_time BIGINT,
    FOREIGN KEY (account_id)
...
CREATE TABLE IF NOT EXISTS profile_state(
    ...
    -- Profile name verification value set by bot/admin automation.
    profile_name_verified BOOLEAN,
    -- Manual profile name verification value set by human admin.
    profile_name_verified_manual BOOLEAN,
...
CREATE TABLE IF NOT EXISTS current_account_media(
    ...
    security_content_set_unix_time BIGINT,
...
CREATE TABLE IF NOT EXISTS chat_state(
    ...
    backup_transfer_byte_count   BIGINT        NOT NULL DEFAULT 0,
    backup_transfer_byte_count_reset_unix_time BIGINT,
    FOREIGN KEY (account_id)
        REFERENCES account_id (id)
            ON DELETE CASCADE
            ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS account_interaction(
    ...
    message_counter_recipient       BIGINT  NOT NULL DEFAULT 0,
    -- Track if video call URL has been created for each side
    video_call_url_created_sender   BOOLEAN NOT NULL DEFAULT FALSE,
    video_call_url_created_recipient BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    FOREIGN KEY (account_id_sender)
        REFERENCES account_id (id)
            ON DELETE CASCADE
            ON UPDATE CASCADE,
    FOREIGN KEY (account_id_recipient)
        REFERENCES account_id (id)
            ON DELETE CASCADE
            ON UPDATE CASCADE,
    FOREIGN KEY (account_id_block_sender)
        REFERENCES account_id (id)
            ON DELETE CASCADE
            ON UPDATE CASCADE,
    FOREIGN KEY (account_id_block_recipient)
        REFERENCES account_id (id)
            ON DELETE CASCADE
            ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS pending_messages(
    ...
    account_id_recipient            BIGINT NOT NULL,
    ...
    -- UUID generated by message sender
...
CREATE TABLE IF NOT EXISTS latest_seen_message_pending_delivery(
    -- The account viewing/receiving the message
    account_id_viewer       BIGINT NOT NULL,
    -- The account who sent the message
    account_id_sender       BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS chat_report_chat_message(
    ...
    message_recipient_account_id_uuid  BLOB                NOT NULL,
    message_unix_time                  BIGINT              NOT NULL,
    message_number                     BIGINT              NOT NULL,
    message_symmetric_key              BLOB                NOT NULL,
    client_message_bytes               BLOB                NOT NULL,
    server_signed_message_bytes        BLOB                NOT NULL,
    FOREIGN KEY (report_id)
        REFERENCES common_report (id)
            ON DELETE CASCADE
            ON UPDATE CASCADE
