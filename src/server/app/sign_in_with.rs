...
const POSSIBLE_ISS_VALUES_GOOGLE: &[&str] = &[
    "accounts.google.com",
...
pub enum SignInWithGoogleError {
    ...
    #[error("HTTP GET for Google public key response cache control header didn't have max age field")]
...
impl SignInWithManager {
    ...
    pub async fn validate_google_token(&self, token: String) -> Result<GoogleAccountInfo, SignInWithGoogleError> {
        ...
        let mut v =  Validation::new(
            google_public_key.common.algorithm
        ...
        Ok(GoogleAccountInfo { id: data.claims.sub, email: data.claims.email })
    }

    async fn get_google_public_key(&self, wanted_kid: &str) -> Result<Jwk, SignInWithGoogleError> {
        match self.get_google_public_key_from_local_keys(wanted_kid).await? {
            KeyStatus::Found(key) => Ok(key),
            KeyStatus::KeyRefreshNeeded => {
                self.download_google_public_keys_and_get_key(wanted_kid).await
            }
        }
    }

    async fn get_google_public_key_from_local_keys(&self, wanted_kid: &str) -> Result<KeyStatus, SignInWithGoogleError> {
        ...
        match keys.as_ref() {
            ...
            Some(keys) => {
                if Instant::now() >= keys.valid_until_this {
                    ...
                    let jwk = keys.keys
                        .find(&wanted_kid)
                        .ok_or(SignInWithGoogleError::JwkNotFound)?.clone();
    ...
    async fn download_google_public_keys_and_get_key(&self, wanted_kid: &str) -> Result<Jwk, SignInWithGoogleError> {
        let download_request = reqwest::Request::new(Method::GET, self.config.sign_in_with_urls().google_public_keys.clone());
        ...
        let possible_header = r.headers().typed_try_get::<CacheControl>().into_error(SignInWithGoogleError::ParsingCacheControlHeader)?;
        let cache_header = possible_header.ok_or(SignInWithGoogleError::MissingCacheControlHeader)?;
        let max_age = cache_header.max_age().ok_or(SignInWithGoogleError::InvalidCacheControlHeader)?;
        let valid_until_this = Instant::now().checked_add(max_age).ok_or(SignInWithGoogleError::CacheCalculation)?;

        let jwk_set: JwkSet = r.json().await.into_error(SignInWithGoogleError::JwkSetParsingFailed)?;
        let mut key_store = self.google_public_keys.write().await;
        *key_store = Some(GooglePublicKeys { keys: jwk_set.clone(), valid_until_this });

        let jwk = jwk_set.find(&wanted_kid).ok_or(SignInWithGoogleError::JwkNotFound)?.clone();
        Ok(jwk)
    }
